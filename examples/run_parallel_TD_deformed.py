...
def single_realization(delta_theta, w_ind):
    ...
    xcr = XCrystal(XCr_path+'/config/C400_TD_9p8keV_ThermalBump1.yaml', omega)
...
if __name__ == '__main__':
    ...
    xcr = XCrystal(XCr_path+'/config/C400_TD_9p8keV_ThermalBump1.yaml', omega0)
    xtools = tools.XCrTools(xcr)

    xcr.tgrid = 10400
 
    xcr.tmax = 1000 * 1.0e-15
    sigma_t =2.0e-15
    ...
    #E0 = np.exp(-(t - xcr.tmax / 2.0-0)**2 / 2.0 / sigma_t**2)+ 0*np.exp(-(t - xcr.tmax / 2.0 - 100.0e-15)**2 / 2.0 / sigma_t**2)
